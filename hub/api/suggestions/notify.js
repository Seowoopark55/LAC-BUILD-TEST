...
export default async function handler(req, res) {
  ...
  let suggestionId = '';
  try {
    suggestionId = String(req.body?.suggestion_id || '').trim();
    if (!UUID_RE.test(suggestionId)) {
      return res.status(400).json({ error: '건의 ID가 올바르지 않습니다.' });
    }
    ...
    const target = await callAxeProductRpc(token, 'platform_suggestion_notification_target', {
      p_suggestion_id: suggestionId,
    });
    const discordUserId = String(target?.discord_user_id || '').trim();
    if (!DISCORD_ID_RE.test(discordUserId)) {
      await callAxeProductRpc(token, 'platform_suggestion_mark_dm_result', {
        p_suggestion_id: suggestionId,
        p_sent: false,
    ...
    if (token && UUID_RE.test(suggestionId)) {
      await callAxeProductRpc(token, 'platform_suggestion_mark_dm_result', {
        p_suggestion_id: suggestionId,
        p_sent: false,
